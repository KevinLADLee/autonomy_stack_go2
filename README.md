...
```
If you want the UTLidar power-off topic to be sent automatically at boot, the related boot assets are grouped under [autostart/README.md](autostart/README.md). Use the helper script below once. It installs and enables the service from `autostart/services/` and the service runs `autostart/scripts/switch_off_utlidar.sh` after startup while waiting for the matching ROS subscriber before publishing.
```
bash autostart/install_service.sh switch_off_utlidar
```
Useful follow-up commands are below.
```
