...
void CloudReprojectionRosNode::loadParameters()
{
    ...
    cloud_slam_topic_ = declare_topic_parameter(this, "topics.cloud_slam", "cloud_slam_topic", "/odin1/cloud_slam");
    odometry_topic_ = declare_topic_parameter(this, "topics.odometry", "odometry_topic", "/odin1/odometry");
    reprojected_image_topic_ = declare_topic_parameter(this, "topics.reprojected_image", "reprojected_image_topic", "/odin1/reprojected_image");

    const auto output_root = std::filesystem::path(this->declare_parameter<std::string>("output_root", default_output_root().string()));
    std::string calib_file = this->declare_parameter<std::string>("calib_file", (output_root / "calib" / "calib.yaml").string());
...
int main(int argc, char** argv)
{
    ...
    const std::string config_file = temp_node->declare_parameter<std::string>("config_file", (std::filesystem::path(package_share) / "config" / "control_command.yaml").string());
    const std::string calib_file = temp_node->declare_parameter<std::string>("calib_file", (output_root / "calib" / "calib.yaml").string());
...
void CloudReprojectionRosNode::loadParameters()
{
    cloud_slam_topic_ = get_topic_param(pnh_, "topics/cloud_slam", "topics.cloud_slam", "cloud_slam_topic", "/odin1/cloud_slam");
    odometry_topic_ = get_topic_param(pnh_, "topics/odometry", "topics.odometry", "odometry_topic", "/odin1/odometry");
    reprojected_image_topic_ = get_topic_param(pnh_, "topics/reprojected_image", "topics.reprojected_image", "reprojected_image_topic", "/odin1/reprojected_image");
