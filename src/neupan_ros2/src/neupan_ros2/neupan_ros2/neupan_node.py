...
class NeupanCore(Node):
    ...
    def __init__(self) -> None:
        ...
        # Obstacle input configuration
        self.declare_parameter("obstacle_input_type", "laserscan")  # "laserscan" or "pointcloud"
        self.declare_parameter("terrain_map_topic", "/terrain_map")
        self.declare_parameter("reflectivity_threshold", 0.1)  # Points with intensity >= threshold are obstacles
        ...
        # Obstacle input configuration
        self.obstacle_input_type = (
            self.get_parameter("obstacle_input_type")
            .get_parameter_value().string_value
        )
        self.terrain_map_topic = (
            self.get_parameter("terrain_map_topic")
            .get_parameter_value().string_value
        )
        self.reflectivity_threshold = (
            self.get_parameter("reflectivity_threshold")
            .get_parameter_value().double_value
        )
        
        # Validate obstacle_input_type
        if self.obstacle_input_type not in ["laserscan", "pointcloud"]:
            raise ValueError(
                f"Invalid obstacle_input_type: '{self.obstacle_input_type}'. "
                "Must be 'laserscan' or 'pointcloud'"
            )
        ...
        # Create obstacle input subscription based on obstacle_input_type
        scan_qos_profile = QoSProfile(depth=10, reliability=QoSReliabilityPolicy.BEST_EFFORT)
        
        if self.obstacle_input_type == "laserscan":
            self.get_logger().info(
                f"Using LaserScan input from topic: "
                f"{self.get_parameter('scan_topic').get_parameter_value().string_value}"
            )
        ...
        elif self.obstacle_input_type == "pointcloud":
            self.get_logger().info(
                f"Using PointCloud2 input from topic: {self.terrain_map_topic}"
            )
            self.create_subscription(
                PointCloud2,
                self.terrain_map_topic,
    ...
    def terrain_map_callback(self, cloud_msg: PointCloud2) -> Optional[npt.NDArray]:
        """Process PointCloud2 data and update obstacle points in map frame.

        Filters out ground points based on reflectivity (intensity field),
        then converts remaining obstacle points to obstacle_points format.

        Args:
            cloud_msg: PointCloud2 message from /terrain_map topic
        ...
        # Check if point cloud frame_id is already map_frame
        cloud_frame_id = cloud_msg.header.frame_id
        needs_transform = cloud_frame_id != self.map_frame

        if not needs_transform:
            self.get_logger().debug(
                f"Point cloud already in map frame ({self.map_frame}), skipping TF transform",
                throttle_duration_sec=5.0
            )

        try:
            # Read point cloud data using read_points_numpy for better performance
            # Returns a structured numpy array with named fields
            points_array = point_cloud2.read_points_numpy(
                cloud_msg,
            ...
            # Handle both structured array (named fields) and regular 2D array (column indices)
            if points_array.dtype.names is not None:
                # Structured array with named fields
                x_coords = points_array['x']
                y_coords = points_array['y']
                z_coords = points_array['z']
                intensities = points_array['intensity']
            else:
                # Regular 2D array: columns are [x, y, z, intensity] in order
                x_coords = points_array[:, 0]
                y_coords = points_array[:, 1]
                z_coords = points_array[:, 2]
                intensities = points_array[:, 3]

            # Filter: keep points with intensity >= threshold (obstacles)
            obstacle_mask = intensities >= self.reflectivity_threshold
            ...
            if obstacle_count == 0:
                with self._state_lock:
                    self.obstacle_points = None
                self.get_logger().debug(
                    f"No obstacle points after reflectivity filtering "
                    f"(threshold={self.reflectivity_threshold})",
                    throttle_duration_sec=1.0
                )
                return None

            # Extract only obstacle point coordinates (apply mask)
            obstacle_x = x_coords[obstacle_mask]
            obstacle_y = y_coords[obstacle_mask]
            obstacle_z = z_coords[obstacle_mask]

            # Stack coordinates: shape (3, n)
            obstacle_points_3d = np.vstack([obstacle_x, obstacle_y, obstacle_z])

            # Transform to map frame if needed
            if needs_transform:
                try:
                    trans = self.tf_buffer.lookup_transform(
                        self.map_frame, cloud_frame_id, rclpy.time.Time()
                    )
                    ...
                    # Transform 3D points: (3, n) -> (3, n) in map frame
                    transformed_points_3d = rot_matrix @ obstacle_points_3d + trans_matrix
                except tf2_ros.LookupException:
                    self.get_logger().debug(
                        f"Waiting for transform from {cloud_frame_id} to {self.map_frame}",
                        throttle_duration_sec=1.0
                    )
                    return None
                except tf2_ros.ConnectivityException:
                    self.get_logger().warn(
                        f"ConnectivityException: Transform from {cloud_frame_id} "
                        f"to {self.map_frame} not available",
                        throttle_duration_sec=1.0
                    )
                    return None
                except tf2_ros.ExtrapolationException as e:
                    self.get_logger().warn(
                        ...
                        f"Check TF timestamps and buffer size.",
                        throttle_duration_sec=1.0
                    )
                    return None
            else:
                # Already in map frame, use directly
                transformed_points_3d = obstacle_points_3d

            # Extract x, y coordinates only: shape (2, n)
            obstacle_points_2d = transformed_points_3d[:2, :]

            # Lock only for writing shared state
            with self._state_lock:
                self.obstacle_points = obstacle_points_2d

            self.get_logger().info(
                f"Point cloud initialized with {obstacle_count} "
                f"obstacle points (filtered from {total_points} total points)",
                once=True
            )
            ...
            self.get_logger().error(
                f"Error processing point cloud: {e}\n{traceback.format_exc()}"
            )
            return None
