...
GridWorld::GridWorld(rclcpp::Node::SharedPtr nh) : initialized_(false), use_keypose_graph_(false)
{
  ...
  tsp_node_ = rclcpp::Node::make_shared("grid_world_tsp_client");
    ...
    std::vector<int>& ordered_cell_indices, const std::shared_ptr<keypose_graph_ns::KeyposeGraph>& keypose_graph)
{
  ...
  if (!SolveTSPService(distance_matrix, 
                       exploring_cell_positions.size() - 1,
                       false,
                       node_index)) {
    RCLCPP_ERROR(rclcpp::get_logger("grid_world"), "TSP solving failed");
    return global_path;
  }
...
bool GridWorld::SolveTSPService(const std::vector<std::vector<int>>& distance_matrix,
                                 int depot,
                                 bool has_dummy,
                                 std::vector<int>& node_sequence)
{
  if (!tsp_client_ || !tsp_client_->wait_for_service(std::chrono::seconds(2))) {
    RCLCPP_WARN(rclcpp::get_logger("grid_world"), 
  ...
  if (rclcpp::spin_until_future_complete(tsp_node_, future, 
    ...
    if (response->success) {
      ...
      RCLCPP_ERROR(rclcpp::get_logger("grid_world"), 
