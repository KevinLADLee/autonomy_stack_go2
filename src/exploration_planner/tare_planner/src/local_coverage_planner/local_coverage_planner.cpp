...
LocalCoveragePlanner::LocalCoveragePlanner(rclcpp::Node::SharedPtr nh)
  ...
  // Initialize TSP service client
  tsp_node_ = rclcpp::Node::make_shared("local_coverage_planner_tsp_client");
  tsp_client_ = tsp_node_->create_client<tare_tsp_interfaces::srv::SolveTSP>("solve_tsp");
}
...

{
  ...
  if (!SolveTSPService(distance_matrix, start_ind, has_start_end_dummy, path_index)) {
    RCLCPP_ERROR(rclcpp::get_logger("local_coverage_planner"), "TSP solving failed");
...
bool LocalCoveragePlanner::SolveTSPService(const std::vector<std::vector<int>>& distance_matrix,
                                            int depot,
                                            bool has_dummy,
                                            std::vector<int>& node_sequence)
{
  if (!tsp_client_ || !tsp_client_->wait_for_service(std::chrono::seconds(2))) {
    RCLCPP_WARN(rclcpp::get_logger("local_coverage_planner"), 
                "TSP service not available");
    return false;
  }
  
  auto request = std::make_shared<tare_tsp_interfaces::srv::SolveTSP::Request>();
  int n = distance_matrix.size();
  request->matrix_size = n;
  request->depot = depot;
  request->has_dummy = has_dummy;
  
  // Flatten matrix
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < n; j++) {
      request->distance_matrix.push_back(distance_matrix[i][j]);
    }
  }
  
  auto future = tsp_client_->async_send_request(request);
  
  if (rclcpp::spin_until_future_complete(tsp_node_, future, 
      std::chrono::seconds(5)) == rclcpp::FutureReturnCode::SUCCESS) {
    auto response = future.get();
    if (response->success) {
      node_sequence = response->node_sequence;
      return true;
    } else {
      RCLCPP_ERROR(rclcpp::get_logger("local_coverage_planner"), 
                   "TSP solving failed: %s", response->message.c_str());
    }
  }
  return false;
}
